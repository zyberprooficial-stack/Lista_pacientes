...
// Expresión regular para validar cédula venezolana (V o E seguido de 6-8 dígitos)
var cedulaRegex = regexp.MustCompile(`^[VE][0-9]{6,8}$`)
...
func (p *PacienteInput) Validate() []string {
	...
	if len(p.Cedula) > 0 && !cedulaRegex.MatchString(p.Cedula) {
		errors = append(errors, "cedula debe tener formato venezolano (V o E seguido de 6-8 dígitos, ej: V12345678)")
	}
